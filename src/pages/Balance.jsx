...
const CAT_COLORS=["#34D399","#F87171","#FBBF24","#60A5FA","#A78BFA","#F472B6","#34D399","#FB923C","#4ADE80","#E879F9","#94A3B8"];
...
function exportPDF(investments, spendings, totalInvested, totalSpent, netBalance) {
  ...
  const html = `
    ...
    <html>
    <head>
      <title>DhanTrack Report - ${month}</title>
      <style>
        * { margin:0; padding:0; box-sizing:border-box; }
        ...
        td { padding:7px 10px; border:1px solid #e5e7eb; color:#374151; }
    ...
    <body>
      ...
      <div class="summary">
        <div class="box"><div class="box-label">Net Balance</div><div class="box-val ${netBalance>=0?"gold":"red"}">₹${Math.abs(netBalance).toLocaleString("en-IN")}</div></div>
      ...
      <table>
        <tr><th>Date</th><th>Name</th><th>Type</th><th>Amount</th><th>Note</th></tr>
...
export default function Balance({ firestoreData }) {
  ...
  // Pie chart data — invest by type
  const investTypes = ["Stock","Crypto","Mutual Fund","Gold","FD/RD","ETF","Other"];
  const investData = investTypes.map((t, i) => ({
    label: t,
  ...
  return (
    <div style={{opacity:vis?1:0,transform:vis?"none":"translateY(10px)",transition:"all .35s ease"}}>
      ...
      <div className="flex items-center justify-between mb-5">
        ...
        {/* Export PDF button */}
        <button
          onClick={() => exportPDF(investments, spendings, totalInvested, totalSpent, netBalance)}
