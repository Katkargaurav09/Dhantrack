...
// ── AddPanel defined at TOP LEVEL — this is the fix for focus bug ──
// If defined inside another component, React remounts it every render
function AddPanel({ open, onClose, onSave }) {
  ...
  useEffect(() => {
    if (open) { setName(""); setAmount(""); setNote(""); setType("Stock"); setSaving(false); }
  }, [open]);
...
function MonthCard({mk,entries,onClick}){
  ...
  return(<div onClick={()=>has&&onClick(mk)} style={{...card,padding:"12px",cursor:has?"pointer":"default",opacity:has?1:0.4,transition:"all .2s"}}
    ...
    <div className="text-xs font-bold font-mono pt-2" style={{borderTop:`1px solid ${has?"rgba(52,211,153,0.15)":"rgba(255,255,255,0.03)"}`,color:has?"#34D399":"#374151"}}>Total = {fmt(total)}</div>
  </div>);
}
...
function MonthDetail({mk,onChange,investments,onBack,onDelete}){
  ...
  return(<div>
    ...
    <div className="grid grid-cols-2 gap-2 mb-4">
      {[["Invest","#34D399"],["Total","#E5E7EB"]].map(([lbl,col])=>(<div key={lbl} style={{...card,padding:"12px 16px",textAlign:"center"}}><p className="text-xs font-mono mb-1" style={{color:"#6B7280"}}>{lbl}</p><p className="text-base font-bold font-mono" style={{color:col}}>{fmt(total)}</p></div>))}
    </div>
  ...
  return(<div style={{opacity:vis?1:0,transform:vis?"none":"translateY(10px)",transition:"all .35s ease"}}>
    {!activeMonth&&(<>
      <div className="mb-4">
        ...
        <p className="text-xs font-mono mt-0.5" style={{color:"#6B7280"}}>{year} · Total: <span style={{color:"#34D399",fontWeight:600}}>{fmt(totalAll)}</span></p>
      </div>
